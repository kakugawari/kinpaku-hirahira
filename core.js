...
(function (root) {
  ...
  /* ------------------------------------------------------------
     1コマの間隔(秒)
     ------------------------------------------------------------
     上限だけでなく、下限も止める。タブから戻ったときに長い間隔が来るのは
     知られているが、最初の1コマは「前」より古い時刻が渡ってくることがある。
     実測で dt = -4.04秒。そのまま歩幅に掛けると (-4.04 x 60 = -242コマぶん)、
     動いているものが一斉に画面の外へ飛び、全部が「外に出た」と見なされる。
     タイトルの箔が降ってこない画面は、これが原因だった
     ------------------------------------------------------------ */
  function こまの間隔(now, last, 上限) {
    const d = (now - last) / 1000;
    if (!Number.isFinite(d) || d < 0) return 0;
    return Math.min(d, 上限 === undefined ? 0.05 : 上限);
  }

  /* ------------------------------------------------------------
     その大きさで画面を組み直してよいか
     ------------------------------------------------------------
     端末の回転中や、ブラウザが枠を組み直している最中は、画面の大きさが
     一瞬 0 で渡ってくることがある。その値で組み直すと、落ちている箔が
     どれも「画面の外」と見なされて上へ戻され、しばらく何も降ってこない
     画面になる(実測で、8回のうち3回)
     ------------------------------------------------------------ */
  function 使える大きさ(w, h) {
