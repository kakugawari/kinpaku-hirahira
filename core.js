...
(function (root) {
  ...
  /* ------------------------------------------------------------
     ...
     落ちる距離は画面の大きさで変わるが、難易度が端末ごとに変わると
     記録の意味がなくなるので、基準の大きさで固定して数える。
     基準: iPhone 13 相当 (型の大きさ s=124.8px、落下 60〜280px)
     ------------------------------------------------------------ */
  const FALL_MIN_U = 60 / 124.8;    /* 型の座標に直した落下距離 */
  const FALL_MAX_U = 280 / 124.8;
  ...
  /* こぼれの許し幅。受け止めにくい型ほど広く許す。
     係数は実測に合わせた: 8つの型で上手に蒔いたときのこぼれが
     計算値の 26〜87% に収まっていたので、0.95倍 + 2% で全型が名人に届く */
  function spillMaxFor(miss) {
    return Math.round(Math.max(0.22, Math.min(0.65, miss * 0.95 + 0.02)) * 100) / 100;
  }
